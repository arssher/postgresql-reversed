...
extern inline void finalize_aggregate(AggState *aggstate,
									  AggStatePerAgg peragg,
									  AggStatePerGroup pergroupstate,
									  Datum *resultVal, bool *resultIsNull);
extern inline void finalize_aggregates(AggState *aggstate,
									   AggStatePerAgg peragg,
									   AggStatePerGroup pergroup,
									   int currentSet);
extern inline void project_aggregates_and_push(AggState *aggstate);
extern inline void AggPushHashEntry(AggState *aggstate, AggHashEntry entry);
...
agg_puttup_hash_table(AggState *aggstate, TupleTableSlot *outerslot)
{
	...
	/* leave this ever-false if to make benchmarks more, well, fair */
	if (DO_AGGSPLIT_COMBINE(aggstate->aggsplit))
...
agg_push_hash_table(AggState *aggstate)
{
	...
	hash_foreach(aggstate->hashtable->hashtab, aggstate);
	aggstate->agg_done = true;
}
